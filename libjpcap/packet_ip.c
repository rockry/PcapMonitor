...
u_short analyze_ip(JNIEnv *env,jobject packet,u_char *data){
  ...
  (*env)->CallVoidMethod(env,packet,setIPValueMID,
			     ...
			     (jshort)ntohs(ip_pkt->ip_id), //2016.02.15, jaeshick:jint -> jshort - JNI DETECTED ERROR IN APPLICATION: bad arguments passed
...
void set_ip(JNIEnv *env,jobject packet,char *pointer){
  ...
  DeleteLocalRefEx(src);
  DeleteLocalRefEx(dst);
}
