...
void analyze_icmp(JNIEnv *env,jobject packet,u_char *data,u_short clen){
  ...
  (*env)->CallVoidMethod(env,packet,setICMPValueMID,
			     (jbyte)icmp_pkt->icmp_type,(jbyte)icmp_pkt->icmp_code,
			     (jshort)icmp_pkt->icmp_cksum); //2016.02.15, jaeshick: add typecast -  JNI DETECTED ERROR IN APPLICATION: bad arguments passed
  ...
  case ICMP_ROUTERADVERT: /* router advertisement */
	...
	DeleteLocalRefEx(addr_str);
      ...
      DeleteLocalRefEx(addrArray);
      DeleteLocalRefEx(prefArray);
    }
    break;
#endif
...
int set_icmp(JNIEnv *env,jobject packet,char *pointer,jbyteArray data)
{
  ...
  case ICMP_ECHO:
	...
	icmp->icmp_cksum=0;
	icmp->icmp_cksum=in_ipcksum((u_short *)icmp,8+length);
  ...
  case ICMP_PARAMPROB: /*parameter problem */
	...
	icmp->icmp_cksum=0;
	icmp->icmp_cksum=in_ipcksum((u_short *)icmp,12+(ippacket==NULL?0:20));
  ...
  case ICMP_TSTAMP: case ICMP_TSTAMPREPLY: /* timestamp*/
	...
	icmp->icmp_cksum=0;
	icmp->icmp_cksum=in_ipcksum((u_short *)icmp,20);
  ...
  case ICMP_MASKREQ: case ICMP_MASKREPLY:   /* netmask*/
	...
	icmp->icmp_cksum=0;
	icmp->icmp_cksum=in_ipcksum((u_short *)icmp,12);
